...
> **Free ngrok plan:** you get a new URL every time you restart ngrok. Each time, update both `mcp_root_url` in `config.yaml` **and** the callback URL in your GitHub OAuth App (Settings → Developer settings → OAuth Apps → your app → Edit), then restart the MCP server: `docker compose restart risk-mcp-server`.
...
| Service | Description |
|---------|-------------|
| `tavro-postgres` | Database |
| `risk-worker` | Agent card worker (idle, waiting for commands) |
| `tavro-api` | Risk classification API |
| `risk-temporal` | Workflow engine |
| `risk-mcp-server` | MCP server |
