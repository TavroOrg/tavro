...
--     replacing it — nothing is ever dropped. This is intentionally more
--     conservative than a drop+replace: the original guarantee is never
--     absent, even momentarily, and rolling back is just dropping the new
--     constraint, with the original never having moved.
--
...
--
-- DELIBERATELY NOT INCLUDED — see chat/README for reasoning:
...
-- After running, check for any "skipped" NOTICE lines — each names the
-- table and reason (typically orphaned rows or NULL tenant_id/company_id
-- on the child side). Once clean, run the corresponding VALIDATE CONSTRAINT
-- statements to move each FK from NOT VALID to fully enforced.
