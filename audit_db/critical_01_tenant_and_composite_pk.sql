...
--      Postgres generates, so this only guards presence/format)
...
--
-- AFTER RUNNING — promoting NOT VALID checks to fully validated, once
-- check_null_tenant_across_db.sql / check_null_company_id_across_db.sql
-- report zero violations for a table (safe on a live table — takes a
-- lightweight lock, not exclusive):
...
--
-- SCOPE — Section A (tenant_id/company_id presence) covers every table
