...
None of the `core`/`curated`/`raw`/`risk_management` tables have any index beyond the unique indexes needed for dedup. Every tenant/company-filtered list view — which is every list view in a multi-tenant UI — is currently a full sequential scan. Minimum viable fix: composite index on `(company_id, tenant_id)` on every direct-owner table, plus `agent_internal_id` indexes on every `agent_*` child table (Postgres does not auto-index the referencing side of a foreign key, so this has to be explicit even after FKs are added).

### 5. Make agent_internal_id the source of agent id instead of agent_id
Today `core.agents` carries two identifiers: `agent_id` (stable business identity) and `agent_internal_id` (per-version identity under the SCD Type 2 versioning pattern — `is_current`, `valid_from_ts`/`valid_to_ts`). Every child/junction table already keys off `agent_internal_id`, not `agent_id`, so `agent_internal_id` is *already* the de facto identity in practice — this task is about making that official: promote it to the real primary key, stop treating `agent_id` as an identifier in new code, and reserve it purely as a "grouping" attribute across an agent's version history. This is a metadata-only change (no data rewrite) since the unique index already exists.
