...
def _run_extraction(module_path: str, class_name: str, config: dict) -> dict:
    buf              = io.StringIO()
...
async def run_connector(connector_id: str, body: ConnectorRunRequest, background_tasks: BackgroundTasks):
    if connector_id not in CONNECTOR_MAP:
        raise HTTPException(status_code=404, detail=f"Connector '{connector_id}' not found")

    module_path, class_name = CONNECTOR_MAP[connector_id]
    result = await asyncio.to_thread(_run_extraction, module_path, class_name, body.config)
