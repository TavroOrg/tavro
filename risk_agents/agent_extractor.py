...
class AgentMetadataExporter:
    ...
    @classmethod
    def _get_agent_id_from_name(cls, agent_name: str) -> Optional[str]:
        if not agent_name:
            return None
        ...
        prefix = cls.sanitize(lower_input[:4]) if len(lower_input) >= 4 else safe_input

        # Keep SQL extension-free; fuzzy ranking is handled in Python below.
        query = f"""
        SELECT agent_id, agent_name
        FROM {cls.CORE_GLUE_DB_NAME}.agents
        WHERE lower(agent_name) LIKE '%{safe_input}%'
           OR lower(agent_name) LIKE '%{prefix}%'
        LIMIT 200
        """
        ...
        for row in rows:
            ...
            if c_id and c_name:
                candidates.append({"id": c_id, "name": c_name})

        # Python fuzzy matching (final ranking / typo-tolerance)
        names_list = [c["name"] for c in candidates]
        
        # We use token_set_ratio which is best for typos + different word orders
        ...
        if result:
            ...
            print(f"Match Analysis: Input='{clean_input}' -> Found='{matched_name}' (Score: {score:.2f})")
            
            # 80 is a strong threshold for token_set_ratio
            if score >= 80:
    ...
    @classmethod
    def get_agent_card(cls, agent_name: Optional[str] = None, agent_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            if agent_name and not agent_id:
                agent_id = cls._get_agent_id_from_name(agent_name)
            ...
            local_card = cls._read_local_agent_card(agent_name=agent_name, agent_id=agent_id)
            if local_card:
                return local_card
            ...
            return {
                "error": "NOT_FOUND",
                "details": f"No local agent card found for id '{agent_id}' in '{card_dir}'.",
            }
    ...
    def send_payload_async(payload: Dict[str, Any]) -> None:
        def _send():
            try:
                requests.post(
                    "http://localhost:80/classify-risk",
                    json=payload,
                    timeout=2
                )
            except Exception:
    ...
    def create_agent(
        ...
        card = {
            ...
            "knowledge_source": {
                ...
                "access_mechanism": knowledge_source.get("description", ""),
            } if knowledge_source else {},
            "data_source": [
                {
                    ...
                    "target_object_id": t["identifier"],
                    "target_object_name": t["name"],
    ...
    def create_ai_use_case_agent_relationship(
        ...
        check_q = f"SELECT 1 FROM {cls.CORE_GLUE_DB_NAME}.agent_ai_use_cases WHERE identifier = '{ai_use_case_id}' AND agent_id = '{agent_catalog_id}' LIMIT 1"
        is_duplicate = len(cls.execute_select(check_q)) > 0

        # 3. Fetch Target Agent Details
        agent_q = f"SELECT agent_id, agent_internal_id FROM {cls.CORE_GLUE_DB_NAME}.agents WHERE agent_id = '{agent_catalog_id}' AND is_current = true LIMIT 1"
        ...
        metrics_q = f"""
            WITH current_agents AS (
                SELECT agent_internal_id FROM {cls.CORE_GLUE_DB_NAME}.agent_ai_use_cases WHERE identifier = '{ai_use_case_id}'
                UNION
            ...
            WHERE uc.identifier = '{ai_use_case_id}' LIMIT 1
        """
