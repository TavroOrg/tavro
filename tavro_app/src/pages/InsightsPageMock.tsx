...
const getProviderLabel = (agent: AgentData) => displayValue(
...
const makeCategoryDistribution = <T,>(
    items: T[],
    classifier: (item: T) => string,
    palette?: Record<string, Pick<DistributionDatum, 'color' | 'text'>>,
): DistributionDatum[] => {
    const counts = new Map<string, number>();
    for (const item of items) {
        const label = displayValue(classifier(item), 'Unknown');
    ...
    const fallback = [
        ...
        { color: 'bg-slate-500', text: 'text-slate-700' },
    ];
    return Array.from(counts.entries())
        .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))
        .map(([label, count], index) => {
            const style = palette?.[label] ?? fallback[index % fallback.length];
            return {
                label,
                count,
...
const InsightsPageMock: React.FC = () => {
    ...
    getAgentAutonomyBucket(agent) === bucket.key
        ...
        const providerDistribution = makeCategoryDistribution(
    agents,
    getProviderLabel
);
...
export default InsightsPageMock;
