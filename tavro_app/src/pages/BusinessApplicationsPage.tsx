                ...
                <div className="flex items-start justify-between gap-3 mb-4">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap justify-end">
                    <span className="inline-flex items-center gap-1 text-xs font-bold px-3 h-8 rounded-full bg-slate-50 text-slate-600 border border-slate-200">
                      ARE: {app.agent_risk_exposure ?? 0}
                    </span>
                    <span className={`inline-flex items-center gap-1 text-xs font-bold px-3 h-8 rounded-full border ${
                      app.agent_risk_tier === 'Critical' || app.agent_risk_tier === 'High'
                        ? 'bg-red-50 text-red-700 border-red-200'
                        : app.agent_risk_tier === 'Medium'
                        ? 'bg-amber-50 text-amber-700 border-amber-200'
                        : app.agent_risk_tier === 'Low'
                        ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                        : 'bg-slate-50 text-slate-500 border-slate-200'
                    }`}>
                      ART: {app.agent_risk_tier ?? 'None'}
                    </span>
                    <span className="inline-flex items-center gap-1 text-xs font-bold px-3 h-8 rounded-full bg-blue-50 text-blue-700 border border-blue-200">
                      <Bot size={20} /> {app.related_agent_count ?? 0}
                    </span>
                  </div>
                </div>
          ...
          <div className="divide-y divide-slate-100">
            {paged.map(app => (
              ...
              >
                ...
                <div className="text-sm font-semibold text-blue-700">{app.related_agent_count ?? 0}</div>
                <div className="text-sm font-semibold text-slate-700">{app.agent_risk_exposure ?? 0}</div>
                <div>
