...
interface LoadIntegrationsModalProps {
    onClose: () => void;
    onSuccess: () => void;
    companyId?: string;
    companyName?: string;
...
const LoadIntegrationsModal: React.FC<LoadIntegrationsModalProps> = ({
    onClose,
    onSuccess,
    companyId,
    companyName,
    ...
    const handleUpload = async () => {
        ...
        try {
            const result = await businessRelationsApi.uploadIntegrations(validFiles, companyId, companyName);
            setUploadResult({
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-lg flex flex-col max-h-[90vh]">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-violet-50 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400 rounded-xl">
                            <Upload size={18} />
                        </div>
                        <div>
                            <h2 className="text-base font-bold text-slate-800 dark:text-slate-100">Load Integrations</h2>
                            <p className="text-xs text-slate-500 dark:text-slate-400">Upload a CSV file — only <strong>integration_name</strong> is required</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 flex flex-col gap-4 overflow-y-auto flex-1 min-h-0">
                        ...
                        <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-3">
                                <div className="flex-1 flex items-center gap-2 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl px-4 py-3">
                                    ...
                                    <div>
                                        <p className="text-sm font-bold text-emerald-700 dark:text-emerald-300">{uploadResult.uploaded_count} Added / Updated</p>
                    ...
                    {!uploadResult && (
                        <div
                            ...
                            className="relative flex flex-col items-center justify-center gap-3 p-8 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 hover:border-violet-400 dark:hover:border-violet-600 hover:bg-violet-50/50 dark:hover:bg-violet-900/10 transition-all cursor-pointer group"
                        >
                            <div className="p-3 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 group-hover:border-violet-300 dark:group-hover:border-violet-700 transition-colors">
                                <FileText size={28} className="text-slate-400 dark:text-slate-500 group-hover:text-violet-500 dark:group-hover:text-violet-400 transition-colors" />
                            </div>
                            <div className="text-center">
                                <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    Drop CSV files here or <span className="text-violet-600 dark:text-violet-400">browse</span>
                                </p>
                    ...
                    {fileEntries.length > 0 && !uploadResult && (
                        <div className="flex flex-col gap-1.5 max-h-48 overflow-y-auto pr-1">
                            {fileEntries.map((entry, i) => (
                                ...
                                >
                                    <FileText size={15} className={entry.error ? 'text-red-400 shrink-0' : 'text-violet-500 dark:text-violet-400 shrink-0'} />
                ...
                {!uploadResult ? (
                    <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 dark:border-slate-800">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-5 py-2 text-sm font-bold bg-violet-600 hover:bg-violet-700 text-white rounded-xl shadow-sm transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                        >
                            {uploading ? (
                    ...
                    <div className="flex justify-end px-6 py-4 border-t border-slate-100 dark:border-slate-800">
                        <button onClick={onClose} className="px-5 py-2 text-sm font-bold bg-violet-600 hover:bg-violet-700 text-white rounded-xl transition-all">
                            Done
                        </button>
                    </div>
                )}
...
export default LoadIntegrationsModal;
