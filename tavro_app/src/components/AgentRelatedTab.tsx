...
interface AgentRelatedTabProps {
  agent: AgentData;
  mode?: 'applications' | 'processes' | 'use_cases' | 'child_agents' | 'all';
  onCountsChange?: (counts: { applications: number; processes: number; useCases?: number; childAgents?: number }) => void;
...
const AgentRelatedTab: React.FC<AgentRelatedTabProps> = ({
  ...
  const showApplications = mode !== 'processes' && mode !== 'use_cases' && mode !== 'child_agents';
  const showProcesses = mode !== 'applications' && mode !== 'use_cases' && mode !== 'child_agents';
  const showUseCases = mode !== 'applications' && mode !== 'processes' && mode !== 'child_agents';
  ...
  const content = (
      <div className={`${embedded ? '' : 'p-5'} flex flex-col gap-6 ${embedded ? '' : 'flex-1 overflow-y-auto'}`}>
        ...
        {(showProcesses || showChildAgents) && showUseCases && <div className="h-px bg-slate-100 w-full" />}
