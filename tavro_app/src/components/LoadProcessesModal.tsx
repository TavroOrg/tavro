...
interface LoadProcessesModalProps {
    onClose: () => void;
    onSuccess: () => void;
    companyId?: string;
    companyName?: string;
...
const LoadProcessesModal: React.FC<LoadProcessesModalProps> = ({
    onClose,
    onSuccess,
    companyId,
    companyName,
    ...
    const handleUpload = async () => {
        ...
        try {
            const result = await businessRelationsApi.uploadProcesses(validFiles, companyId, companyName);
            setUploadResult({
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-lg flex flex-col max-h-[90vh]">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-base font-bold text-slate-800 dark:text-slate-100">Load Processes</h2>
                            <p className="text-xs text-slate-500 dark:text-slate-400">Upload a CSV file — only <strong>process_name</strong> is required</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            {uploadResult.warnings.length > 0 && (
                                <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl px-4 py-3">
                                    <p className="text-[11px] font-bold text-amber-600 dark:text-amber-400 uppercase tracking-wide mb-2 flex items-center gap-1.5">
                                        <AlertCircle size={11} /> {uploadResult.warnings.length} warning{uploadResult.warnings.length !== 1 ? 's' : ''}
                                    </p>
...
export default LoadProcessesModal;
