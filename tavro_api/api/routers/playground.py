...
async def _enable_azure_foundry_a2a(
    ...
    # Caller only invokes this when _portal_agent_skills(config) is non-empty.
    patch_url = f"{endpoint}/agents/{agent_name}?api-version={api_version}"
    payload = {
        "agent_card": {
            "description": description,
            "version": "1.0",
            "skills": _portal_agent_skills(config),
        },
        "agent_endpoint": {
...
async def _provision_azure_foundry_agent(config: SessionConfig) -> AzureFoundryAgentProvisioning:
    ...
    # Only enable A2A when the portal agent actually has skills defined —
    # no A2A card for agents with nothing to advertise.
    a2a_enabled = bool(_portal_agent_skills(config))
    if a2a_enabled:
