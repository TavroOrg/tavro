...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_table_columns_tenant_id_present') THEN
        ALTER TABLE core.table_columns
            ADD CONSTRAINT chk_table_columns_tenant_id_present
            CHECK (tenant_id IS NOT NULL AND btrim(tenant_id) <> '') NOT VALID;
    END IF;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'table_columns: tenant_id CHECK skipped — %', SQLERRM;
END $$;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_table_columns_company_id_present') THEN
        ALTER TABLE core.table_columns
            ADD CONSTRAINT chk_table_columns_company_id_present
            CHECK (company_id IS NOT NULL AND btrim(company_id) <> '') NOT VALID;
    END IF;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'table_columns: company_id CHECK skipped — %', SQLERRM;
END $$;
