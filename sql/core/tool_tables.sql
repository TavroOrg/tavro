...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_tool_tables_tenant_id_present') THEN
        ALTER TABLE core.tool_tables
            ADD CONSTRAINT chk_tool_tables_tenant_id_present
            CHECK (tenant_id IS NOT NULL AND btrim(tenant_id) <> '') NOT VALID;
    END IF;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'tool_tables: tenant_id CHECK skipped — %', SQLERRM;
END $$;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_tool_tables_company_id_present') THEN
        ALTER TABLE core.tool_tables
            ADD CONSTRAINT chk_tool_tables_company_id_present
            CHECK (company_id IS NOT NULL AND btrim(company_id) <> '') NOT VALID;
    END IF;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'tool_tables: company_id CHECK skipped — %', SQLERRM;
END $$;
