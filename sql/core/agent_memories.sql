...
-- ALTER TABLE core.agent_memories VALIDATE CONSTRAINT fk_agent_memories_agent;
DO $$
BEGIN
...
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'agent_memories: composite FK skipped — %', SQLERRM;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_agent_memories_tenant_id_present') THEN
        ALTER TABLE core.agent_memories
            ADD CONSTRAINT chk_agent_memories_tenant_id_present
            CHECK (tenant_id IS NOT NULL AND btrim(tenant_id) <> '') NOT VALID;
    END IF;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'agent_memories: tenant_id CHECK skipped — %', SQLERRM;
END $$;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_agent_memories_company_id_present') THEN
        ALTER TABLE core.agent_memories
            ADD CONSTRAINT chk_agent_memories_company_id_present
            CHECK (company_id IS NOT NULL AND btrim(company_id) <> '') NOT VALID;
    END IF;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'agent_memories: company_id CHECK skipped — %', SQLERRM;
END $$;
