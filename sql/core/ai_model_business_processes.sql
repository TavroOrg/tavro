...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_ai_model_business_processes_tenant_id_present') THEN
        ALTER TABLE core.ai_model_business_processes
            ADD CONSTRAINT chk_ai_model_business_processes_tenant_id_present
            CHECK (tenant_id IS NOT NULL AND btrim(tenant_id) <> '') NOT VALID;
    END IF;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'ai_model_business_processes: tenant_id CHECK skipped — %', SQLERRM;
END $$;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_ai_model_business_processes_company_id_present') THEN
        ALTER TABLE core.ai_model_business_processes
            ADD CONSTRAINT chk_ai_model_business_processes_company_id_present
            CHECK (company_id IS NOT NULL AND btrim(company_id) <> '') NOT VALID;
    END IF;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'ai_model_business_processes: company_id CHECK skipped — %', SQLERRM;
END $$;
